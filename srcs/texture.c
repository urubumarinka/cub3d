...
// Load XPM texture from file
int load_texture(void *mlx, t_tex_img *texture, const char *path)
{
	texture->img_ptr = mlx_xpm_file_to_image(mlx, (char *)path, &texture->width, &texture->height);
	if (!texture->img_ptr)
	{
	...
	texture->data = (uint32_t *)mlx_get_data_addr(texture->img_ptr, &texture->bpp, &texture->line_length, &texture->endian);
	if (!texture->data)
	{
